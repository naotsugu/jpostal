...
public interface TownEditor {
    ...
    static List<TownEditor> standardEditors() {
        return Arrays.asList(
            ...
            (town, org) -> town.endsWith("一円") && org.city.endsWith(town.replace("一円", "")) ? empty : keep(town),
            ...
            // 59 records
            (town, org) -> town.endsWith("（丁目）") ? removeBrackets(town) : keep(town),

            // 1 records
            (town, org) -> town.endsWith("（各町）") ? removeBrackets(town) : keep(town),

            // 36 records
            (town, org) -> town.endsWith("（番地）") ? removeBrackets(town) : keep(town),
            ...
            // 1 records
            (town, org) -> town.endsWith("以下）") ? removeBrackets(town) : keep(town),

            // 6 records
            (town, org) -> town.endsWith("以上）") ? removeBrackets(town) : keep(town),

            // 1 records
            (town, org) -> town.endsWith("以内）") ? removeBrackets(town) : keep(town),

            // 2 records
            (town, org) -> town.endsWith("以降）") ? removeBrackets(town) : keep(town),

            // 1 records
            (town, org) -> town.endsWith("以外）") ? removeBrackets(town) : keep(town),

            // 2 records
            (town, org) -> town.endsWith("を含む）") ? removeBrackets(town) : keep(town),

            // 1 records. 愛知県新城市
            (town, org) -> town.endsWith("○○屋敷）") ? removeBrackets(town) : keep(town),
            ...
            (town, org) -> town.contains("番地以上") ? removeBrackets(town) : keep(town),

            // 13 records
            (town, org) -> town.contains("「") && town.contains("」") ? removeSqBrackets(town) : keep(town),

            // ex) 西瑞江（４丁目１～２番・１０～２７番、５丁目）-> 西瑞江
            (town, org) -> town.matches(".*（.*・.*）.*") ? removeBrackets(town) : keep(town),
    ...
    static List<TownEditor> simpleEditors() {
        return Arrays.asList(
                ...
                (town, org) -> town.contains("地割、") ? Arrays.asList(town.split("、")) : keep(town),
                (town, org) -> town.startsWith("甲、乙") ? Arrays.asList(town.split("、")) : keep(town),
                (town, org) -> hasBrackets(town) ? removeBrackets(town) : keep(town)
        );
    }
    ...
    private static boolean hasBrackets(String input) {
        return input.contains("（") && input.contains("）");
    }

    private static List<String> extractBrackets(String input) {
    ...
    private static List<String> removeBrackets(String input) {
